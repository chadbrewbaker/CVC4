...
apt-get update -y
apt-get install -y  openjdk-7-jdk swig m4 libgmp-dev libboost-dev libboost-thread-dev libcln-dev antlr3 libantlr3c-dev
...
make -j3
find . -name "*.jar" >> envdata
find .  -name "*.class" >> envdata
find . -name "*.so" >> envdata
find .  -name "*.dyld" >> envdata
#apt-get install -y valgrind
touch CVC4.tar
#cp ./builds/x86_64-unknown-linux-gnu/debug-noproof/src/bindings/java/.libs/libcvc4jni.so ./
#cp ./builds/x86_64-unknown-linux-gnu/debug-noproof/src/bindings/java/classes/edu/nyu/acsys/CVC4/*.class ./
